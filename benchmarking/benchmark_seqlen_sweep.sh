...
CUDA_VISIBLE_DEVICES=$GPU_ID vllm bench sweep serve \
    ...
    --bench-cmd "vllm bench serve --backend openai --model ${MODEL} --dataset-name random --num-prompts 100 --save-result --num-warmups 20 --ignore-eos --max-concurrency 1" \
    --num-runs 3 \
    --bench-params "./bench_seqlens_params.json" \
    --output-dir $OUTDIR
